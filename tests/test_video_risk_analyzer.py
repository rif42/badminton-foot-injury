...
from badminton_risk.video_risk_analyzer import (
    _extract_pose,
    _landmark_to_point,
    _status,
    analyze_video,
...
def test_main_prints_expected_output(capsys):
    ...
    with (
        patch("cv2.VideoCapture") as mock_cap,
        patch("badminton_risk.video_risk_analyzer._create_pose_detector", return_value=mock_pose),
    ):
        mock_cap_instance = MagicMock()
        mock_cap_instance.read.side_effect = [(True, fake_frame), (False, None)]
        mock_cap_instance.get.side_effect = lambda key: (
...
def test_main_on_synthetic_video(tmp_path):
    ...
    with patch("badminton_risk.video_risk_analyzer._create_pose_detector", return_value=mock_pose):
